...
from .schemas import AnalysisRequest, AnalysisJobResponse, AnalysisStatusResponse, AnalysisResults

logger = logging.getLogger(__name__)

def update_job_status(job_id: str, status: str, message: str = None, error_details: Dict[str, Any] = None):
    """Update job status in database"""
    with SessionLocal() as session:
        try:
            job = session.query(PremiumAnalysisJob).filter(PremiumAnalysisJob.premium_job_id == job_id).first()
            if job:
                job.status = status
                if error_details:
...
def wait_for_workload_ready(deployment_name: str, namespace: str) -> Dict[str, Any]:
    """Local mode - simulate workload ready"""
    logger.info(f"Local mode: Simulating workload ready for {deployment_name}")
    return {
        "status": "ready",
        "pod_name": f"analysis-{job_id[:8]}-pod"
    }
...
def wait_for_kubescape_analysis(job_id: str) -> Dict[str, Any]:
    ...
    return {
        ...
        "results": {
            "vulnerabilities": [
                {"id": "CVE-2023-1234", "severity": "high", "description": "Simulated vulnerability"},
                {"id": "CVE-2023-5678", "severity": "medium", "description": "Simulated vulnerability"}
            ]
        }
    }
...
def process_kubescape_vex(job_id: str, analysis_results: Dict[str, Any]) -> Dict[str, Any]:
...
def convert_vex_statements_to_reachability(reachability_results: Dict[str, Any]) -> Dict[str, Any]:
...
def generate_analysis_summary(job_id: str, analysis_results: Dict[str, Any]) -> Dict[str, Any]:
