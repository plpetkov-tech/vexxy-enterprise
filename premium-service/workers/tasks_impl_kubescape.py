...
def extract_kubescape_results(
    ...
    # Store as evidence and generate VEX ID
    vex_id = None
    if vex_document:
        _, vex_id = evidence_storage.store_vex_document(job_id, vex_document)
        # Handle case where statements field is null (Go nil slice marshals to JSON null)
        statements = vex_document.get('statements') or []
        logger.info(f"Stored runtime VEX document (ID: {vex_id}): {len(statements)} statements")
    else:
